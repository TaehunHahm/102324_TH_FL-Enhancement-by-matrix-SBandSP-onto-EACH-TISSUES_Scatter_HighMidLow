...
Sublimation_Tissues <- read.csv("102324_Spraying_Tissues_scatter(FITC,TRITC,mCherry)Brain.csv")
...
scatter_plot <- ggplot(Sublimation_Tissues, aes(x = C, y = log2FoldEnhancement, size = -log10(pvalue), color = keyvals)) +
  ...
  scale_color_manual(values = c('low' = '#6495ED', 'mid' = '#FFA500', 'high' = '#DC143C'),  # More distinctive colors
  ...
  labs(
    title = "FL Enhancement by Matrix Spraying onto Brain Tissues",
  ...
  annotate("rect", xmin = -Inf, xmax = Inf, ymin = 1, ymax = 4, alpha = .1, fill = "#FFFF00") +
  annotate("text", x = 2, y = 3.0, label = "High Interest", hjust = 0.5, vjust = 1, color = "#DC143C")  # Use 2 for TRITC position
